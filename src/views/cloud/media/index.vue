<template>
	<div class="media">
		<a-card
			...
			:bodyStyle="{ height: 'calc(100% - 60px)', padding: '0 24px' }"
		>
			<a-button
				type="primary"
				shape="circle"
						...
						>
							<div
								...
								:style="
									...
									'/public/xuanque/' +
									item.path +
									');' /*height:&quot;+(item.ratio>1.5?&quot;184px&quot;:&quot;400px&quot;)*/
								"
							/>
						</a-list-item>
					</transition-group>
	...
	export default {
		...
		methods: {
			...
			handleChange(e) {
				...
				if (
					"done|removed".indexOf(e.file.status) > -1 &&
					e.file.response.code == 0
				) {
					console.log(e);
					this.fileList = e.fileList;
				}
				if (e.file.status !== "removed" && e.file.response.code !== 0) {
					if (e.file.response && e.file.response.code == -2) {
						this.$message.error("错误的文件类型", 1);
					} else {
			...
			preview(e) {
				...
				this.$axios
					.get("/public/image/" + e.response.data.imgPath)
					.then((res) => {
						// this.previewImage = res
					});
				console.log(e);
				this.previewImage =
					progress.env.NODE_ENV +
					"/public/image/" +
					e.response.data.imgPath;
...
.imgitem {
	background: url(http://localhost:9200/public/image/1593226339645-9006.jpg)
		no-repeat center;
