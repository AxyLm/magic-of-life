				...
				<a-col :span="10" :xs="24" :xl="10">
					<a-spin tip="Loading..." :spinning="treeLoading">
						<a-tree
							...
							:defaultE-expanded-keys="[
								null,
								'5f2395dc50402e70802c16c0',
							]"
							@select="selTree"
							@dragstart="dragstart"
							@dragend="dragend"
								...
								<span style="float: right">
									<a-button
										...
										@click.prevent.stop="
											change(item, 'add')
										"
										><span style="color: #1890ff !important"
											>添加</span
										></a-button
									>
									<a-popconfirm
										title="确定删除么?"
										@confirm="delRoute(item)"
					...
					<a-spin tip="Loading" :spinning="formLoading">
						...
						>
							<transition-group name="slide-fade" mode="out-in">
								...
								<a-form-item
									label="权限(visibleRoles)"
									:labelCol="{ span: 5 }"
									key="6"
								>
									<a-select
										:value="routerInfo.visibleRoles"
										@change="authChange"
										mode="multiple"
	...
	export default {
		...
		methods: {
			...
			delRoute(item) {
				...
				this.$axios
					.post("/users/delrouter", {
						routerId: item.id,
					})
					.then((res) => {
						this.$message.success({
							content: item.title + "   删除成功",
							duration: 2.5,
						});
						this.initTree();
					});
			...
			initTree() {
				...
				this.$axios
					.post("/users/getAuthRouter", { flag: 1 })
					.then((res) => {
						if (res.code == 0) {
							...
							this.expandedKeys = this.expandAllTree(this.treeData);
						}
			...
			expandAllTree(tree) {
				...
				tree.forEach((item) => {
					...
					if (
						...
						item[this.replaceFields.children].length > 0
					) {
						allKeys = allKeys.concat(
							this.expandAllTree(item[this.replaceFields.children])
						);
					}
				});
				return allKeys;
			...
			onDrop(event) {
				...
				this.$axios.post("/users/updateRoute", roteInfo).then((res) => {
					if (res.code == 0) {
			...
			selTree(selectedKeys, e) {
				...
				this.$axios
					.post("/users/queryRouter", { id, parent })
					.then((res) => {
						if (res.code == 0) {
